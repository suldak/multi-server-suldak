...
public class TtToLiRepositoryImpl implements TtToLiRepositoryCustom, BridgeInterface {
    ...
    public Boolean deleteByLiquorPriKey(Long liquorPriKey) {
        jpaQueryFactory
                .delete(ttToLi)
                .where(ttToLi.liquor.id.eq(liquorPriKey))
                .execute();
        return true;
    }
    ...
    public List<Long> findLiquorPriKeyByTagPriKey(List<Long> tagPriKeys) {
        return jpaQueryFactory
                ...
                .from(ttToLi)
                .innerJoin(ttToLi.liquor, liquor)
                .on(ttToLi.liquor.id.eq(liquor.id))
                .innerJoin(ttToLi.tasteType, tasteType)
                .on(
                        tagPriKeys == null || tagPriKeys.isEmpty() ?
                                ttToLi.tasteType.id.eq(tasteType.id) :
